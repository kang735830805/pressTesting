...
func syncTps(num int, ctx context.Context, clients []*sdk.ChainClient) (err error) {
	m := sync.Map{}

	sNum := 0
	pMap := make(map[string]string)
	err = json.Unmarshal([]byte(parameter), &pMap)
	for i := 0 ; i < num; i++ {
		if sNum > len(clients)-1 {
			sNum = 0
		}
		InvoceChaincode(clients[i], loop, name, method, pMap, m)
		sNum++
	}
	return nil
}
...
func InvoceChaincode(client *sdk.ChainClient, loop int, name, method string, args map[string]string, m sync.Map){	var txid string = ""
	for i := 0; i < loop; i++ {
		txid = sdkop.UserContractAssetInvoke(client, name, method,"1", "",args, false) //最后一个参数为是否同步获取交易结果？
